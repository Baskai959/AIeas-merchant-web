...
export interface LiveRoomEnvelope<T = Record<string, unknown>> {
  type: string;
  requestId?: string;
  seq?: number;
  ack?: boolean;
...
export interface LiveRoomClientOptions {
  roomId: string | number;
  onStatusChange?: (status: LiveRoomConnectionStatus) => void;
  onMessage?: (message: LiveRoomEnvelope) => void;
...
function buildWebSocketUrl(roomId: string | number, lastSeq?: number) {
  const protocol = window.location.protocol === 'https:' ? 'wss:' : 'ws:';
  const url = new URL(
    `${protocol}//${window.location.host}/ws/live-rooms/${roomId}`
  );
...
export default class LiveRoomClient {
  private options: LiveRoomClientOptions;
  ...
  constructor(options: LiveRoomClientOptions) {
  ...
  connect() {
    ...
    try {
      this.socket = new WebSocket(
        buildWebSocketUrl(this.options.roomId, this.lastSeq)
      );
    } catch (error) {
  ...
  private send(message: LiveRoomEnvelope) {
  ...
  private handleMessage(rawData: string) {
    try {
      const message = JSON.parse(rawData) as LiveRoomEnvelope;
  ...
  private updateStatus(status: LiveRoomConnectionStatus) {
    this.options.onStatusChange?.(status);
  }

  private buildRequestId(action: string) {
    return `${action}-${this.options.roomId}-${Date.now()}`;
  }
}
