...
import { OrderDeal, OrderPayStatus, OrderStatus } from '@/services/orders';
...
const PAY_STATUS_META: Record<OrderPayStatus, { label: string; color: string }> = {
...
function getLotTitle(lot?: AuctionLot, item?: Item) {
  if (!lot) {
    return '直播拍品';
  }
  return item?.title || '未命名拍品';
}
    ...
    try {
      ...
      if (sessionResult.status === 'ENDED') {
        const nextReportTask = await fetchLiveAnalysisReportTask(
          sessionResult.id,
          { liveRoomId: sessionResult.liveRoomId }
        ).catch(() => undefined);
      ...
      const itemResults = await Promise.all(
        itemIds.map((itemId) => fetchItem(itemId).catch(() => undefined))
      );
  ...
  const lotColumns = [
    {
      ...
      render: (_: unknown, record: AuctionLot) => {
        ...
        const title = getLotTitle(record, item);
        return (
  ...
  const bidColumns = [
    {
      ...
      render: (_: unknown, record: LiveSessionBidRecord) => {
        ...
        const item = lot ? items[String(lot.itemId)] : undefined;
        return getLotTitle(lot, item);
      },
    },
    {
      title: '出价人',
      ...
      render: (_: unknown, record: LiveSessionBidRecord) => {
        const source = record as Record<string, unknown>;
        return (
          pickString(source, ['nickname', 'bidderName', 'userName', 'buyerName']) ||
          '竞拍用户'
        );
      },
    },
        ...
        <Space>
          {session ? (
            <>
              <Button
                onClick={() =>
                  history.push(`/live-rooms/${session.liveRoomId}/sessions`)
                }
              >
                返回直播记录
              </Button>
              <Button
                type="primary"
                onClick={() =>
                  history.push(`/live-rooms/${session.liveRoomId}/workbench`)
                }
              >
                返回工作台
              </Button>
      ...
      {loadError ? (
        <Card>
          <AppState
            ...
            subtitle={loadError}
            actionText={
              loadError.includes('不存在') ? undefined : '重新加载'
            }
            onAction={
        ...
        <Space direction="vertical" size={16} style={{ width: '100%' }}>
              ...
              <Col xs={24} lg={8}>
                <div className={styles.sessionSummaryPanel}>
                  <div className={styles.sessionStatusLine}>
                    <span className={styles.sessionSummaryLabel}>场次状态</span>
          ...
          <Card className={styles.tableCard}>
            <Tabs>
              <TabPane key="lots" title={`场次拍品（${lots.length}）`}>
                <Table
                  rowKey="auctionId"
                  columns={lotColumns}
                  data={lots}
