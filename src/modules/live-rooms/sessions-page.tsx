...
export default function LiveRoomSessionsPage() {
  ...
  async function loadSessions(
    ...
    try {
      const [roomResult, sessionResult] = await Promise.all([
        room ? Promise.resolve(room) : fetchLiveRoom(id),
        listLiveSessionsByRoom(id, {
  ...
  return (
    <AppPage
      ...
      extra={
        <Space>
          <Button onClick={() => history.push(`/live-rooms/${id}/workbench`)}>
            返回工作台
          </Button>
    ...
    >
      <Card className={styles.filterCard}>
        ...
        >
          <Form.Item field="status" label="场次状态">
            <Select
              allowClear
              placeholder="全部状态"
      ...
      <Card className={styles.tableCard}>
        {loadError ? (
          ...
          <AppState
            status="empty"
            title={hasFilters ? '未找到匹配场次' : '暂无直播记录'}
            subtitle={
              hasFilters
                ? '可以调整场次状态后重试。'
                : room
            ...
            onAction={
              hasFilters
                ? handleReset
                : () => history.push(`/live-rooms/${id}/workbench`)
            }
          />
        ) : (
          <Table
